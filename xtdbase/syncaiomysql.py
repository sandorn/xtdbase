        ...
        if hasattr(self, 'engine') and self.engine is not None:
            try:
                ...
                if hasattr(self.engine, '_pool') and self.engine._pool is not None:
                    ...
                    mylog.info('self.__del__ | 连接池已经清理')
            except Exception as e:
                mylog.warning(f'self.__del__ | 清理连接池时发生异常: {e}')
            ...
            try:
                ...
                mylog.info('self.__del__ | 数据引擎已关闭')
            except Exception as e:
                mylog.warning(f'self.__del__ | 关闭引擎时发生异常: {e}')
            finally:
                self.engine = None
        ...
        if hasattr(self, 'loop') and self.loop is not None:
            try:
                if not self.loop.is_closed():
                    ...
                    self.loop.close()
                    mylog.info('self.__del__ | 事件循环已关闭')
            except Exception as e:
                mylog.warning(f'self.__del__ | 关闭事件循环时发生异常: {e}')
            finally:
                self.loop = None
...
if __name__ == '__main__':
    ...
    def run_test():
        ...
        try:
            ...
            # 2. 测试查询操作 - 使用格式化结果
            mylog.info('测试查询操作(格式化结果)...')
            query_sql = 'SELECT * FROM users2 LIMIT 5'
            results = aio.query(query_sql)  # 默认返回格式化后的字典列表
            mylog.success(f'查询结果数量: {len(results)}')
            if results:
                mylog.success(f'第一条记录(格式化后): {results[0]}')
                # 可以直接通过列名访问数据
                if isinstance(results[0], dict):
                    mylog.success(f'格式化结果示例: ID={results[0].get("ID")}, username={results[0].get("username")}')

            # 测试查询操作 - 使用原始结果
            mylog.info('测试查询操作(原始结果)...')
            raw_results = aio.query(query_sql, return_dict=False)
            if raw_results:
                mylog.info(f'第一条原始记录: {raw_results[0]}')
            ...
            if inserted_id:
                ...
                try:
                    ...
                    mylog.success(f'更新结果: {update_result}')
                    ...
                    if updated_data and isinstance(updated_data, list) and len(updated_data) > 0 and isinstance(updated_data[0], dict):
                        mylog.info(f'更新后的数据: {updated_data[0]}')
                        mylog.info(f'用户名已更新为: {updated_data[0].get("username")}, 会员级别已更新为: {updated_data[0].get("会员级别")}')
                except Exception as e:
                    mylog.fail(f'处理更新操作时发生错误: {e}')
            else:
                mylog.warning('未能获取到插入数据的ID')
            ...
            mylog.info(f'清理结果: {cleanup_result}')
        except Exception as e:
            mylog.fail(f'测试过程中发生错误: {e}')
            import traceback
