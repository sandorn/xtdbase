...
class RedisManager:
    ...
    def run_until_loop(self, tasks: list[Coroutine[Any, Any, Any]]) -> None:
        """运行异步任务直到完成
        ...
        for coro in tasks:
            if self.loop.is_running():
                ...
                created_tasks.append(asyncio.create_task(coro))
            else:
    ...
    def __enter__(self) -> RedisManager:
        """支持上下文管理器的入口方法"""
        logger.info(f'进入Redis上下文: {self.host}:{self.port}/{self.db}')
        return self
    ...
    def __exit__(self, exc_type: type[BaseException] | None, exc_val: BaseException | None, exc_tb: Any) -> None:
        ...
        Args:
            ...
            exc_tb: 异常追踪信息
        """
        logger.info(f'退出Redis上下文: {self.host}:{self.port}/{self.db}')

        # 如果发生异常,记录异常信息
        if exc_type is not None:
            logger.error(f'上下文中发生异常: {exc_type.__name__}: {exc_val}')
        ...
        if self.client:
            if isinstance(self.client, aioredis.Redis):
                ...
                async def close_async():
                    if self.client and isinstance(self.client, aioredis.Redis):
                        await self.client.aclose()

                try:
                    self.run_until_loop([close_async()])
                    logger.info('异步Redis客户端已关闭')
                except Exception as e:
                    logger.error(f'关闭异步Redis客户端失败: {e!s}')
            else:
                # 同步客户端直接关闭
                self.client.close()
